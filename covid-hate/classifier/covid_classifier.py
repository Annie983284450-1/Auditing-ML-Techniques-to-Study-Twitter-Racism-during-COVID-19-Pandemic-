...
def bert_feat_batch(tweet_batch, model, cuda=False):
    ...
    tok = torch.tensor(tok_arr[:, :512]).to(torch.int64)
    msk = torch.tensor(msk_arr[:, :512]).to(torch.int64)
    seg = torch.tensor(seg_arr[:, :512]).to(torch.int64)
    ...
    with torch.no_grad():
        ...
        # encoded_layers = model(tok, seg, msk)
